...
namespace
{
    static const char *sIds[ESM3::MagicEffect::Length] =
    {
        "WaterBreathing",
        "SwiftSwim",
        "WaterWalking",
...
namespace ESM3
{
    ...
    void MagicEffect::load(Reader& reader, bool& isDeleted)
    {
        isDeleted = false; // MagicEffect record can't be deleted now (may be changed in the future)

        // vanilla MW accepts the _SND subrecords before or after DESC.
        while (reader.getSubRecordHeader())
        {
            ...
            switch (subHdr.typeId)
            {
                ...
                case ESM3::SUB_MEDT:
                {
                    assert (subHdr.dataSize == 36 && "MGEF incorrect data size");
                    reader.get(mData);
                    if (reader.getFormat() == 0)
                    {
                        ...
                        if (mIndex < NumberOfHardcodedFlags)
                        mData.mFlags |= HardcodedFlags[mIndex];
                    }
                    break;
                }
                case ESM3::SUB_ITEX: reader.getZString(mIcon); break;
                case ESM3::SUB_PTEX: reader.getZString(mParticle); break;
                case ESM3::SUB_BSND: reader.getZString(mBoltSound); break;
                case ESM3::SUB_CSND: reader.getZString(mCastSound); break;
                case ESM3::SUB_HSND: reader.getZString(mHitSound);  break;
                case ESM3::SUB_ASND: reader.getZString(mAreaSound); break;
                case ESM3::SUB_CVFX: reader.getZString(mCasting);   break;
                case ESM3::SUB_BVFX: reader.getZString(mBolt); break;
                case ESM3::SUB_HVFX: reader.getZString(mHit);  break;
                case ESM3::SUB_AVFX: reader.getZString(mArea); break;
    ...
    void MagicEffect::save(ESM::ESMWriter& esm, bool /*isDeleted*/) const
    {
    ...
    short MagicEffect::getResistanceEffect(short effect)
    {
        ...
        if (effects.empty())
        {
            ...
            for (int i=0; i<5; ++i)
                effects[DrainAttribute+i] = ResistMagicka; // FIXME: Warning C4244
            for (int i=0; i<5; ++i)
