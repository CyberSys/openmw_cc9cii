...
namespace MWMechanics
{
    ...
    bool Pickpocket::getDetected(float valueTerm)
    {
        ...
        float pcSneak = static_cast<float>(mThief.getClass().getSkill(mThief, ESM3::Skill::Sneak));
        int iPickMinChance = MWBase::Environment::get().getWorld()->getStore().get<ESM3::GameSetting>()
                .find("iPickMinChance")->mValue.getInteger();
        int iPickMaxChance = MWBase::Environment::get().getWorld()->getStore().get<ESM3::GameSetting>()
    ...
    bool Pickpocket::pick(const MWWorld::Ptr& item, int count)
    {
        ...
        float fPickPocketMod = MWBase::Environment::get().getWorld()->getStore().get<ESM3::GameSetting>()
