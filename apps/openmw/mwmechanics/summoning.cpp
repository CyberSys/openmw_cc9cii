...
namespace MWMechanics
{
    ...
    {
        return ((effectId >= ESM3::MagicEffect::SummonScamp && effectId <= ESM3::MagicEffect::SummonStormAtronach)
             || (effectId == ESM3::MagicEffect::SummonCenturionSphere)
             || (effectId >= ESM3::MagicEffect::SummonFabricant && effectId <= ESM3::MagicEffect::SummonCreature05));
    }
    ...
    std::string getSummonedCreature(int effectId)
    {
        ...
        {
            {ESM3::MagicEffect::SummonAncestralGhost, "sMagicAncestralGhostID"},
            {ESM3::MagicEffect::SummonBonelord, "sMagicBonelordID"},
            {ESM3::MagicEffect::SummonBonewalker, "sMagicLeastBonewalkerID"},
            {ESM3::MagicEffect::SummonCenturionSphere, "sMagicCenturionSphereID"},
            {ESM3::MagicEffect::SummonClannfear, "sMagicClannfearID"},
            {ESM3::MagicEffect::SummonDaedroth, "sMagicDaedrothID"},
            {ESM3::MagicEffect::SummonDremora, "sMagicDremoraID"},
            {ESM3::MagicEffect::SummonFabricant, "sMagicFabricantID"},
            {ESM3::MagicEffect::SummonFlameAtronach, "sMagicFlameAtronachID"},
            {ESM3::MagicEffect::SummonFrostAtronach, "sMagicFrostAtronachID"},
            {ESM3::MagicEffect::SummonGoldenSaint, "sMagicGoldenSaintID"},
            {ESM3::MagicEffect::SummonGreaterBonewalker, "sMagicGreaterBonewalkerID"},
            {ESM3::MagicEffect::SummonHunger, "sMagicHungerID"},
            {ESM3::MagicEffect::SummonScamp, "sMagicScampID"},
            {ESM3::MagicEffect::SummonSkeletalMinion, "sMagicSkeletalMinionID"},
            {ESM3::MagicEffect::SummonStormAtronach, "sMagicStormAtronachID"},
            {ESM3::MagicEffect::SummonWingedTwilight, "sMagicWingedTwilightID"},
            {ESM3::MagicEffect::SummonWolf, "sMagicCreature01ID"},
            {ESM3::MagicEffect::SummonBear, "sMagicCreature02ID"},
            {ESM3::MagicEffect::SummonBonewolf, "sMagicCreature03ID"},
            {ESM3::MagicEffect::SummonCreature04, "sMagicCreature04ID"},
            {ESM3::MagicEffect::SummonCreature05, "sMagicCreature05ID"}
        };

        auto it = summonMap.find(effectId);
        if (it != summonMap.end())
            return MWBase::Environment::get().getWorld()->getStore().get<ESM3::GameSetting>().find(it->second)->mValue.getString();
        return std::string();
    }
    ...
    void UpdateSummonedCreatures::visit(EffectKey key, int effectIndex, const std::string &sourceName, const std::string &sourceId, int casterActorId, float magnitude, float remainingTime, float totalTime)
    {
        ...
        {
            mActiveEffects.insert(ESM3::SummonKey(key.mId, sourceId, effectIndex));
        }
    }
    ...
    void UpdateSummonedCreatures::process(bool cleanup)
    {
        ...
        std::map<ESM3::SummonKey, int>& creatureMap = creatureStats.getSummonedCreatureMap();

        for (std::set<ESM3::SummonKey>::iterator it = mActiveEffects.begin(); it != mActiveEffects.end(); ++it)
        {
        ...
        for (std::map<ESM3::SummonKey, int>::iterator it = creatureMap.begin(); it != creatureMap.end(); )
        {
    ...
    void purgeSummonEffect(const MWWorld::Ptr& summoner, const std::pair<const ESM3::SummonKey, int>& summon)
    {
